...
const AllBranches = () => {
    ...
    const setBranchesFunc = async (page=1) => {
        window.scrollTo(0,0)
        try {
            let {response, data} = await customFetcher(`congregation/cottages/get/?page=${page}&search=${search}`)
            if (data.data) {
    ...
    return ( 
        <div className="w-full pt-24 pb-12 text-slate-900 overflow-auto px-5 text-center">
            <h1 className="text-3xl font-semibold text-slate-900">
            <FontAwesomeIcon icon={faCross} /> Branches</h1>
            <div className="w-full flex justify-center mt-3 mb-5">
                ...
                onClick={() => {setBranchesFunc()}}><FontAwesomeIcon icon={faMagnifyingGlass} /> 
                </div>
            </div>
            </div>
            {branches.length > 0 && <div className="overflow-x-auto shadow-md rounded-md mt-3">
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        {branches.map((branch) => (<tr className="bg-white border-b dark:bg-gray-800
                         dark:border-gray-700" key={branch.id}>
                            ...
                            <td className="py-4 px-6">
                                <div className="w-full">
                                    ...
                                    bg-green-600 text-slate-100 p-1 hover:bg-green-700">
                                        View</a></Link>
                                </div>
                            </td>
                            <td className="py-4 px-6">
            ...
            <ul className="inline-flex items-center -space-x-px">
                {meta.prev_page && <li onClick={() => {setBranchesFunc(meta.prev_page)}}>
                ...
                {pageNumbers.map((page) => (<li key={page} onClick={() => {setBranchesFunc(page)}}>
                ...
                {meta.next_page && <li onClick={() => {setBranchesFunc(meta.next_page)}}>
            ...
            {branches.length > 0 && <div className="w-full text-center font-semibold text-lg mt-3">
                <h3>{meta.last_item} out of {meta.num_items}</h3>
            </div>}
            {!branches.length > 0 && <div className="w-full text-center">
