...
const AllBranchElders = ({ branch }) => {
    const [branchElders, setBranchElders] = useState([])
    ...
    const setBranchEldersFunc = async (page=1) => {
        window.scrollTo(0,0)
        try {
            let {response, data} = await customFetcher(`cottage/elders/get/${branch.id}/?page=${page}&search=${search}`)
            setBranchElders(data.data)
            setMeta(data.meta)
    ...
    return ( 
        <div className="w-full pt-24 pb-12 text-slate-900 overflow-auto px-5">
            ...
            <FontAwesomeIcon icon={faCross} /> {branch.cottage_name} Elders</h1>
            <div className="w-full flex justify-center mt-3 mb-5">
                ...
                onClick={() => {setBranchEldersFunc()}}><FontAwesomeIcon icon={faMagnifyingGlass} /> 
                </div>
            </div>
            </div>
            {branchElders.length > 0 && <div className="overflow-x-auto shadow-md rounded-md mt-3">
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        {branchElders.map((elders) => (<tr className="bg-white border-b dark:bg-gray-800
            ...
            <ul className="inline-flex items-center -space-x-px">
                {meta.prev_page && <li onClick={() => {setBranchEldersFunc(meta.prev_page)}}>
                ...
                {pageNumbers.map((page) => (<li key={page} onClick={() => {setBranchEldersFunc(page)}}>
                ...
                {meta.next_page && <li onClick={() => {setBranchEldersFunc(meta.next_page)}}>
            ...
            {branchElders.length > 0 && <div className="w-full text-center font-semibold text-lg mt-3">
                <h3>{meta.last_item} out of {meta.num_items}</h3>
            </div>}
            {!branchElders.length > 0 && <div className="w-full text-center">
...
export default AllBranchElders;