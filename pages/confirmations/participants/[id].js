...
const AllConfirmationParticipants = ({ confirmation }) => {
    const [confirmationParticipants, setConfirmationParticipants] = useState([])
    const [search, setSearch] = useState(" ")
    const [noValue] = useState("No result found")
    const [meta, setMeta] = useState({})
    const [ pageNumbers, setPageNumbers ] = useState([])

    const setConfirmationParticipantsFunc = async (page=1) => {
        window.scrollTo(0,0)
        try {
            let {response, data} = await customFetcher(`confirmation/participant/get/${confirmation.id}/?page=${page}&search=${search}`)
            setConfirmationParticipants(data.data)
            setMeta(data.meta)
    ...
    useEffect(() => {
        if (!localStorage.getItem('church-access-token')) {
            ...
            setConfirmationParticipantsFunc()
        }
    }, [])
    ...
    return ( 
        <div className="w-full pt-24 pb-12 text-slate-900 overflow-auto px-5">
            ...
            <FontAwesomeIcon icon={faDove} /> {confirmation.confirmation_name} Participants</h1>
            <div className="w-full flex justify-center mt-3 mb-5">
                ...
                onClick={() => {setConfirmationParticipantsFunc()}}><FontAwesomeIcon icon={faMagnifyingGlass} /> 
                </div>
            </div>
            </div>
            {confirmationParticipants.length > 0 && <div className="overflow-x-auto shadow-md rounded-md mt-3">
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        {confirmationParticipants.map((confirmationParticipant) => (<tr className="bg-white 
                        border-b dark:bg-gray-800
                         dark:border-gray-700" key={confirmationParticipant.id}>
                            <th scope="row" className="py-4 px-6 font-medium text-gray-900 
                            whitespace-nowrap dark:text-white">
                                {confirmationParticipant.participant_name}
                            </th>
                            <td className="py-4 px-6">
                                <div className="w-full">
                                    <Link href={`/confirmations/participants/update/${confirmationParticipant.id}`}><a 
                            ...
                            <td className="py-4 px-6">
                                <div className="w-full">
                                    <Link href={`/confirmations/participants/delete/${confirmationParticipant.id}`}><a 
            ...
            <ul className="inline-flex items-center -space-x-px">
                {meta.prev_page && <li onClick={() => {setConfirmationParticipantsFunc(meta.prev_page)}}>
                ...
                {pageNumbers.map((page) => (<li key={page} onClick={() => {setConfirmationParticipantsFunc(page)}}>
                ...
                {meta.next_page && <li onClick={() => {setConfirmationParticipantsFunc(meta.next_page)}}>
            ...
            {confirmationParticipants.length > 0 && <div className="w-full text-center font-semibold text-lg mt-3">
                <h3>{meta.last_item} out of {meta.num_items}</h3>
            </div>}
            {!confirmationParticipants.length > 0 && <div className="w-full text-center">
                <h2 className="text-slate-900 font-semibold text-xl">{noValue}</h2>
            </div>}
            <div className="w-full mt-5 text-center">
                <div className="w-full">
                    <Link href={`/confirmations/participants/create/${confirmation.id}`}><a className="rounded-md 
...
export default AllConfirmationParticipants;