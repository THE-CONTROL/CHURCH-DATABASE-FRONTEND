...
const AllBaptismParticipants = ({ baptism }) => {
    const [baptismParticipants, setBaptismParticipants] = useState([])
    const [search, setSearch] = useState(" ")
    const [noValue] = useState("No result found")
    const [meta, setMeta] = useState({})
    const [ pageNumbers, setPageNumbers ] = useState([])

    const setBaptismParticipantsFunc = async (page=1) => {
        window.scrollTo(0,0)
        try {
            let {response, data} = await customFetcher(`baptism/participant/get/${baptism.id}/?page=${page}&search=${search}`)
            setBaptismParticipants(data.data)
            setMeta(data.meta)
    ...
    return ( 
        <div className="w-full pt-24 pb-12 text-slate-900 overflow-auto px-5">
            ...
            <FontAwesomeIcon icon={faWater} /> {baptism.baptism_name} Participants</h1>
            <div className="w-full flex justify-center mt-3 mb-5">
                ...
                onClick={() => {setBaptismParticipantsFunc()}}><FontAwesomeIcon icon={faMagnifyingGlass} /> 
                </div>
            </div>
            </div>
            {baptismParticipants.length > 0 && <div className="overflow-x-auto shadow-md rounded-md mt-3">
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        {baptismParticipants.map((baptismParticipant) => (<tr className="bg-white 
                        border-b dark:bg-gray-800
                         dark:border-gray-700" key={baptismParticipant.id}>
                            <th scope="row" className="py-4 px-6 font-medium text-gray-900 
                            whitespace-nowrap dark:text-white">
                                {baptismParticipant.participant_name}
                            </th>
                            <td className="py-4 px-6">
                                <div className="w-full">
                                    <Link href={`/baptisms/participants/update/${baptismParticipant.id}`}><a 
                            ...
                            <td className="py-4 px-6">
                                <div className="w-full">
                                    <Link href={`/baptisms/participants/delete/${baptismParticipant.id}`}><a 
            ...
            <ul className="inline-flex items-center -space-x-px">
                {meta.prev_page && <li onClick={() => {setBaptismParticipantsFunc(meta.prev_page)}}>
                ...
                {pageNumbers.map((page) => (<li key={page} onClick={() => {setBaptismParticipantsFunc(page)}}>
                ...
                {meta.next_page && <li onClick={() => {setBaptismParticipantsFunc(meta.next_page)}}>
            ...
            {baptismParticipants.length > 0 && <div className="w-full text-center font-semibold text-lg mt-3">
                <h3>{meta.last_item} out of {meta.num_items}</h3>
            </div>}
            {!baptismParticipants.length > 0 && <div className="w-full text-center">
                <h2 className="text-slate-900 font-semibold text-xl">{noValue}</h2>
            </div>}
            <div className="w-full mt-5 text-center">
                <div className="w-full">
                    <Link href={`/baptisms/participants/create/${baptism.id}`}><a className="rounded-md 
...
export default AllBaptismParticipants;