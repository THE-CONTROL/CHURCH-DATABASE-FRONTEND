...
const AllDistrictMembers = ({ district }) => {
    ...
    const setDistrictMembersFunc = async (page=1) => {
        window.scrollTo(0,0)
        try {
            let {response, data} = await customFetcher(`district/members/get/${district.id}/?page=${page}&search=${search}`)
            setDistrictMembers(data.data)
            setMeta(data.meta)
    ...
    return ( 
        <div className="w-full pt-24 pb-12 text-slate-900 overflow-auto px-5">
            ...
            <FontAwesomeIcon icon={faEarthAfrica} /> {district.district_name} Members</h1>
            <div className="w-full flex justify-center mt-3 mb-5">
                ...
                onClick={() => {setDistrictMembersFunc()}}><FontAwesomeIcon icon={faMagnifyingGlass} /> 
                </div>
            </div>
            </div>
            {districtMembers.length > 0 && <div className="overflow-x-auto shadow-md rounded-md mt-3">
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        {districtMembers.map((members) => (<tr className="bg-white border-b dark:bg-gray-800
            ...
            <ul className="inline-flex items-center -space-x-px">
                {meta.prev_page && <li onClick={() => {setDistrictMembersFunc(meta.prev_page)}}>
                ...
                {pageNumbers.map((page) => (<li key={page} onClick={() => {setDistrictMembersFunc(page)}}>
                ...
                {meta.next_page && <li onClick={() => {setDistrictMembersFunc(meta.next_page)}}>
            ...
            {districtMembers.length > 0 && <div className="w-full text-center font-semibold text-lg mt-3">
                <h3>{meta.last_item} out of {meta.num_items}</h3>
            </div>}
            {!districtMembers.length > 0 && <div className="w-full text-center">
...
export default AllDistrictMembers;