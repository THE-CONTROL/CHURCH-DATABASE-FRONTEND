...
const DeleteService = ({ showFlashFunc, service }) => {
  const router = useRouter()

  const deleteService = async () => {
    try {
      let {response, data} = await customFetcher(`service/delete/${service.id}`, 
      { method: "DELETE"})
    ...
    return ( 
        <div className="w-screen h-screen pt-24 overflow-auto">
            ...
            p-2">
                <h1 className="text-xl font-bold text-red-700 text-center">Deleting this service 
                will result in permanent loss of all data associated with this service.</h1>
                <h2 className="text-lg font-semibold text-slate-900 text-center pt-2">Are you sure you 
                want to delete this service?</h2>
                <div className="w-full mt-4">
                    ...
                    <div className="w-full rounded-b-lg text-center text-slate-900 mt-2">
                        <h1 className="font-medium text-xl">{service.service_name}</h1>
                    </div>
                    <div className="w-full p-1 flex justify-center mt-2">
                    <Link href="/service/all"><a className="rounded-md bg-blue-600 text-slate-100 mr-2
                    ...
                    <a className="rounded-md bg-red-600 text-slate-100
                        ...
                        h-8 hover:h-9" onClick={() => {deleteService()}}>Delete</a>
                    </div>
                </div>
            </div>
        </div>
