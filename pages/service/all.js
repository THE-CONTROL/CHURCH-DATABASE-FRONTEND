...
const AllService = () => {
    ...
    const setServiceFunc = async (page=1) => {
        window.scrollTo(0,0)
        try {
            let {response, data} = await customFetcher(`congregation/service/get/?page=${page}&search=${search}`)
            if (data.data) {
    ...
    return ( 
        <div className="w-full pt-24 pb-12 text-slate-900 overflow-auto px-5">
            ...
            <FontAwesomeIcon icon={faPeopleRoof} /> Service</h1>
            <div className="w-full flex justify-center mt-3 mb-5">
                ...
                onClick={() => {setServiceFunc()}}><FontAwesomeIcon icon={faMagnifyingGlass} /> 
                </div>
            </div>
            </div>
            {service.length > 0 && <div className="overflow-x-auto shadow-md rounded-md mt-3">
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        {service.map((service) => (<tr className="bg-white border-b dark:bg-gray-800
                         dark:border-gray-700" key={service.id}>
                            ...
                            <td className="py-4 px-6">
                                {service.head_minister}
                            </td>
                            <td className="py-4 px-6">
                                {service.assistant_minister}
                            </td>
                            <td className="py-4 px-6">
                                {service.no_men}
                            </td>
                            ...
                            <td className="py-4 px-6">
                                <div className="w-full">
                                    <Link href={`/service/update/${service.id}`}><a 
                            ...
                            <td className="py-4 px-6">
                                <div className="w-full">
                                    <Link href={`/service/delete/${service.id}`}><a 
            ...
            <ul className="inline-flex items-center -space-x-px">
                {meta.prev_page && <li onClick={() => {setServiceFunc(meta.prev_page)}}>
                ...
                {pageNumbers.map((page) => (<li key={page} onClick={() => {setServiceFunc(page)}}>
                ...
                {meta.next_page && <li onClick={() => {setServiceFunc(meta.next_page)}}>
            ...
            {service.length > 0 && <div className="w-full text-center font-semibold text-lg mt-3">
                <h3>{meta.last_item} out of {meta.num_items}</h3>
            </div>}
            {!service.length > 0 && <div className="w-full text-center">
                <h2 className="text-slate-900 font-semibold text-xl">{noValue}</h2>
            </div>}
            <div className="w-full mt-5 text-center">
                <div className="w-full">
                    <Link href="/service/create"><a className="rounded-md bg-blue-600 text-slate-100
