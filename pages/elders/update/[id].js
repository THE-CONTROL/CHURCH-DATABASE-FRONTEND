...
export const getStaticPaths = async () => {
  try {
    const res = await fetch("https://church-database-backend.onrender.com/elder/special/get")
    const data = await res.json()

    const paths = data.map(elder => {
      return {
        params: {id: elder.id.toString()}
      }
    })
...
export const getStaticProps = async (context) => {
  try {
    ...
    const res = await fetch(`https://church-database-backend.onrender.com/elder/special/get/${id}`)
    const data = await res.json()
...
const UpdateElder = ({ showFlashFunc, elder }) => {
    const [elderName, setElderName] = useState("")
    const [sex, setSex] = useState("Female")
    const [maritalStatus, setMaritalStatus] = useState("Single")
    const [telephone, setTelephone] = useState("")
    const [elderType, setElderType] = useState(false)
  ...
  const updateElder = async () => {
    let body = {
      ...
      try {
        let {response, data} = await customFetcher(`elder/update/${elder.id}`, 
    ...
    const setelder = () => {
      try {
        ...
        setSex(elder.sex)
        setDateJoined(elder.date_joined)
        setMaritalStatus(elder.marital_status)
        setTelephone(elder.telephone)
        setElderType(elder.elder_type)
        setElderPost(elder.elder_post)
      }
      catch (error) {showFlashFunc("Request failed, check your network connection and try again!", false)}
    }
    ...
    return ( 
      ...
      <div className="w-64 sm:w-96 m-auto rounded-3xl">
        <div className="w-11/12 m-auto p-2">
          ...
          <h1 className="text-center text-3xl font-semibold text-white">Update Elder</h1>
        </div>
        <div className='w-11/12 m-auto h-56 overflow-auto pb-5'>
            ...
            onChange={(e) => { setElderName(e.target.value) }} value={elderName}/>
            </div>
            <div className="w-11/12 bg-white m-auto mt-6 flex justify-end shadow-gray-900 shadow-sm
            hover:shadow-md hover:shadow-gray-900 rounded-md">
            <div className="w-2/12 text-gray-900 text-center text-xl pt-2 rounded-md">
            ...
            bg-white border-l-2 focus:border-2 focus:border-gray-900 focus:outline-0 rounded-md"
            onChange={(e) => { setElderPost(e.target.value) }} value={elderPost}/>
            </div>
            <div className="w-11/12 bg-white m-auto mt-6 flex justify-end shadow-gray-900 shadow-sm
            hover:shadow-md hover:shadow-gray-900 rounded-md">
            <div className="w-2/12 text-gray-900 text-center text-xl pt-2 rounded-md">
            ...
            bg-white border-l-2 focus:border-2 focus:border-gray-900 focus:outline-0 rounded-md"
            onChange={(e) => { setElderType(e.target.value) }} value={elderType}/>
            </div>
            <div className="w-11/12 bg-white m-auto mt-6 flex justify-end shadow-gray-900 shadow-sm
            hover:shadow-md hover:shadow-gray-900 rounded-md">
            <div className="w-2/12 text-gray-900 text-center text-xl pt-2 rounded-md">
        ...
        shadow-gray-900 shadow-sm hover:shadow-md hover:shadow-gray-900">
          ...
          hover:bg-yellow-700"
          onClick={() => { updateElder() }}>Update elder</button>
        </div>
      </div>
    </div>
     );
