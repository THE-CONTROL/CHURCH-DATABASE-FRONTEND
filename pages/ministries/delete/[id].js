...
const DeleteMinistry = ({ showFlashFunc, ministry }) => {
  const router = useRouter()

  const deleteMinistry = async () => {
    try {
      let {response, data} = await customFetcher(`ministry/delete/${ministry.id}`, 
      { method: "DELETE"})
    ...
    return ( 
        <div className="w-screen h-screen pt-24 overflow-auto">
            ...
            p-2">
                <h1 className="text-xl font-bold text-red-700 text-center">Deleting this ministry will 
                result in permanent loss of all data associated with this ministry.</h1>
                <h2 className="text-lg font-semibold text-slate-900 text-center pt-2">Are you sure you 
                want to delete this ministry?</h2>
                <div className="w-full mt-4">
                    ...
                    <div className="w-full rounded-b-lg text-center text-slate-900 mt-2">
                        <h1 className="font-medium text-xl">{ministry.ministry_name}</h1>
                    </div>
                    <div className="w-full p-1 flex justify-center mt-2">
                    <Link href="/ministries/all"><a className="rounded-md bg-blue-600 text-slate-100 mr-2
                    ...
                    <a className="rounded-md bg-red-600 text-slate-100
                        ...
                        h-8 hover:h-9" onClick={() => {deleteMinistry()}}>Delete</a>
                    </div>
                </div>
            </div>
        </div>
