...
const AllMinistryMembers = ({ ministry }) => {
    ...
    const setMinistryMembersFunc = async (page=1) => {
        window.scrollTo(0,0)
        try {
            let {response, data} = await customFetcher(`ministry/members/get/${ministry.id}/?page=${page}&search=${search}`)
            setMinistryMembers(data.data)
            setMeta(data.meta)
    ...
    return ( 
        <div className="w-full pt-24 pb-12 text-slate-900 overflow-auto px-5">
            ...
            <FontAwesomeIcon icon={faUsers} /> {ministry.ministry_name} Members</h1>
            <div className="w-full flex justify-center mt-3 mb-5">
                ...
                onClick={() => {setMinistryMembersFunc()}}><FontAwesomeIcon icon={faMagnifyingGlass} /> 
                </div>
            </div>
            </div>
            {ministryMembers.length > 0 && <div className="overflow-x-auto shadow-md rounded-md mt-3">
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        {ministryMembers.map((ministryMember) => (<tr className="bg-white border-b 
                        dark:bg-gray-800
                         dark:border-gray-700" key={ministryMember.id}>
                            <th scope="row" className="py-4 px-6 font-medium text-gray-900 
                            whitespace-nowrap dark:text-white">
                                {ministryMember.participant_name}
                            </th>
                            <td className="py-4 px-6">
                                {ministryMember.date_joined}
                            </td>
                            <td className="py-4 px-6">
                                <div className="w-full">
                                    <Link href={`/ministries/members/update/${ministryMember.id}`}><a 
                            ...
                            <td className="py-4 px-6">
                                <div className="w-full">
                                    <Link href={`/ministries/members/delete/${ministryMember.id}`}><a 
            ...
            <ul className="inline-flex items-center -space-x-px">
                {meta.prev_page && <li onClick={() => {setMinistryMembersFunc(meta.prev_page)}}>
                ...
                {pageNumbers.map((page) => (<li key={page} onClick={() => {setMinistryMembersFunc(page)}}>
                ...
                {meta.next_page && <li onClick={() => {setMinistryMembersFunc(meta.next_page)}}>
            ...
            {ministryMembers.length > 0 && <div className="w-full text-center font-semibold text-lg mt-3">
                <h3>{meta.last_item} out of {meta.num_items}</h3>
            </div>}
            {!ministryMembers.length > 0 && <div className="w-full text-center">
                <h2 className="text-slate-900 font-semibold text-xl">{noValue}</h2>
            </div>}
            <div className="w-full mt-5 text-center">
                <div className="w-full">
                    <Link href={`/ministries/members/create/${ministry.id}`}><a className="rounded-md 
...
export default AllMinistryMembers;