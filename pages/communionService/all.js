...
import { faBreadSlice, faMagnifyingGlass } from '@fortawesome/free-solid-svg-icons'


const AllCommunionService = () => {
    const [communionService, setCommunionService] = useState([])
    const [search, setSearch] = useState(" ")
    const [noValue] = useState("No result found")
    const [meta, setMeta] = useState({})
    const [ pageNumbers, setPageNumbers ] = useState([])

    const setCommunionServiceFunc = async (page=1) => {
        window.scrollTo(0,0)
        try {
            let {response, data} = await customFetcher(`congregation/communion_services/get?page=${page}&search=${search}`)
            if (data.data) {
                setCommunionService(data.data)
                setMeta(data.meta)
    ...
    return ( 
        <div className="w-full pt-24 pb-12 text-slate-900 overflow-auto px-5">
            ...
            <FontAwesomeIcon icon={faBreadSlice} /> Communion Service</h1>
            <div className="w-full flex justify-center mt-3 mb-5">
                ...
                onClick={() => {setCommunionServiceFunc()}}><FontAwesomeIcon icon={faMagnifyingGlass} /> 
                </div>
            </div>
            </div>
            {communionService.length > 0 && <div className="overflow-x-auto shadow-md rounded-md mt-3">
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        {communionService.map((communionService) => (<tr className="bg-white border-b dark:bg-gray-800
                         dark:border-gray-700" key={communionService.id}>
                            <th scope="row" className="py-4 px-6 font-medium text-gray-900 
                            whitespace-nowrap dark:text-white">
                                {communionService.service_name}
                            </th>
                            <td className="py-4 px-6">
                                {communionService.service_date}
                            </td>
                            <td className="py-4 px-6">
                                {communionService.service_head_count}
                            </td>
                            <td className="py-4 px-6">
                                <div className="w-full">
                                    <Link href={`/communionService/participants/${communionService.id}`}>
                            ...
                            <td className="py-4 px-6">
                                <div className="w-full">
                                    <Link href={`/communionService/update/${communionService.id}`}><a 
                            ...
                            <td className="py-4 px-6">
                                <div className="w-full">
                                    <Link href={`/communionService/delete/${communionService.id}`}><a 
            ...
            <ul className="inline-flex items-center -space-x-px">
                {meta.prev_page && <li onClick={() => {setCommunionServiceFunc(meta.prev_page)}}>
                ...
                {pageNumbers.map((page) => (<li key={page} onClick={() => {setCommunionServiceFunc(page)}}>
                ...
                {meta.next_page && <li onClick={() => {setCommunionServiceFunc(meta.next_page)}}>
            ...
            {communionService.length > 0 && <div className="w-full text-center font-semibold text-lg mt-3">
                <h3>{meta.last_item} out of {meta.num_items}</h3>
            </div>}
            {!communionService.length > 0 && <div className="w-full text-center">
                <h2 className="text-slate-900 font-semibold text-xl">{noValue}</h2>
            </div>}
            <div className="w-full mt-5 text-center">
                <div className="w-full">
                    <Link href="/communionService/create"><a className="rounded-md bg-blue-600 text-slate-100
...
export default AllCommunionService;