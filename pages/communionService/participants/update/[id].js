...
export const getStaticPaths = async () => {
  try {
    const res = await fetch("http://127.0.0.1:8000/communion_service/special/get")
...
export const getStaticProps = async (context) => {
  try {
    ...
    const res = await fetch(`http://church-database-backend.onrender.com/communion_service/special/get/${id}`)
    const data = await res.json()
...
const UpdateCommunionServiceParticipant = ({ showFlashFunc, communionServiceParticipant }) => {
    const [participantName, setParticipantName] = useState("")
    const [participantType, setParticipantType] = useState("New")
    const router = useRouter()
  

  const updateCommunionServiceParticipant = async () => {
      let body = {
        participant_name: participantName.trim(),
        participant_type: participantType.trim(),
      }
      try {
        let {response, data} = await customFetcher(`communion_participants/update/${communionServiceParticipant.id}`, 
    ...
    const setCommunionServiceParticipant = () => {
      try {
        setParticipantName(communionServiceParticipant.participant_name)
        setParticipantType(communionServiceParticipant.participant_type)
      }
      catch (error) {showFlashFunc("Request failed, check your network connection and try again!", false)}
    }
    ...
    useEffect(() => {
      if (!localStorage.getItem('church-access-token')) {
        ...
        setCommunionServiceParticipant()
      }
    }, [])
        ...
        <div className="w-64 sm:w-96 m-auto rounded-3xl">
          <div className="w-11/12 m-auto p-2">
            ...
            <h1 className="text-center text-3xl font-semibold text-white">
              Create <FontAwesomeIcon icon={faBreadSlice} /> {communionServiceParticipant.service_name} participant</h1>
          </div>
          <div className='w-11/12 m-auto h-36 overflow-auto'>
              ...
              bg-white border-l-2 focus:border-2 focus:border-gray-900 focus:outline-0 rounded-md"
              onChange={(e) => { setParticipantType(e.target.value) }} value={participantType}>
          ...
          shadow-gray-900 shadow-sm hover:shadow-md hover:shadow-gray-900">
            ...
            hover:bg-yellow-700"
            onClick={() => { updateCommunionServiceParticipant() }}>Update participant</button>
          </div>
        </div>
      </div>
     );
...
export default UpdateCommunionServiceParticipant;